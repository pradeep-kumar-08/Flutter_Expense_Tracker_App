...
class FirebaseExpenseRepo implements ExpenseRepository {
  ...
  Future<List<Expense>> getExpenses(String uid) async {
    try {
     return await expenseCollection.where("uid", isEqualTo: uid)
      .get()
      .then((value) => value.docs.map((e)=>
  ...
  Future<void> createUser(AppUser appUser) async{
    try {
      await userCollection
          .doc(appUser.uid)
          .set(appUser.toEntity().toDocument());
    } catch (e) {
      log(e.toString());
      rethrow;
    }
  ...
  @override
  Future<void> updateUser(String uid, DateTime lastLogin) async {
    try {
      await userCollection
          .doc(uid)
          .update({'lastLogInDate' : lastLogin});
    } catch (e) {
      log(e.toString());
      rethrow;
    }
